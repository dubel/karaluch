...
import { ARENA_HALF, BOT_SPAWN, CEMETERY, CHAPEL, PLAYER_SPAWN, TREE_HEIGHT, WINDMILL_PROP, WORKSHOP, WORKSHOP_RADIUS } from './config'
...
function scatter(
  ...
  for (let z = start; z <= limit; z += cell) {
    for (let x = start; x <= limit; x += cell) {
      ...
      if (Math.abs(px - CEMETERY.x) < CEMETERY.hx + 3.2 && Math.abs(pz - CEMETERY.z) < CEMETERY.hz + 3.2) continue
